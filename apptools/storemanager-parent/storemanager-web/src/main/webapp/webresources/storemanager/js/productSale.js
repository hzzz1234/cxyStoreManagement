...
function deleteMulti(){
    ...
    if(!confirm("确认要删除？,数据不可恢复！")){
        ...
        $.ajax({
            type:"POST",
            url:"/productsale/saleDeleteMulti",
            data:{ids:ids},
            dataType:"json",
            beforeSend:function(){
                debugger;
...
function reQuery(){
    document.location.assign("/productsale/saleQueryLikeKeys?"+"keys=&activePage=1"+"&startNum=1&pageSize="+$("#selectValue").val());
}
...
function queryByKeys(){
    ...
    document.location.assign("/productsale/saleQueryLikeKeys?"+"keys="+$("#search-input").val()+"&activePage=1"+"&startNum=1&pageSize="+$("#selectValue").val());
