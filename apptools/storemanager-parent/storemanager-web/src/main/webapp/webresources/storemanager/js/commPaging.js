...
function setPage(container, count, pageindex) {
    ...
    var pageClick = function() {
        ...
        oAlink[1].onclick = function() { //点击上一页
            ...
            inx--;
            document.location.assign("/productpurchase/purchaseQueryLikeKeys?"+"keys="+$("#search-input").val()+"&activePage="+inx+"&startNum="+(inx-1)*10+"&pageSize=10");
        ...
        oAlink[oAlink.length - 2].onclick = function() { //点击下一页
            ...
            document.location.assign("/productpurchase/purchaseQueryLikeKeys?"+"keys="+$("#search-input").val()+"&activePage="+(parseInt(inx)+1)+"&startNum="+inx*10+"&pageSize=10");
            //setPage(container, count, inx);
            return false;
        }
        oAlink[oAlink.length - 1].onclick = function() { //点击末页
            ...
            document.location.assign("/productpurchase/purchaseQueryLikeKeys?"+"keys="+$("#search-input").val()+"&activePage="+count+"&startNum="+(count-1)*10+"&pageSize=10");
