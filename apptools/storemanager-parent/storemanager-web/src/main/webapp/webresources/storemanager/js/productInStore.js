...
function deleteOne(id){
    ...
    if(!confirm("确认要删除？,数据不可恢复！")){
        ...
        $.ajax({
            type:"GET",
            url:"/productinstore/houseInDeleteOne",
            data:{id:id},
            dataType:"json",
            beforeSend:function(){
                debugger;
...
function deleteMulti(){
    ...
    if(!confirm("确认要删除？,数据不可恢复！")){
        ...
        $.ajax({
            type:"POST",
            url:"/productinstore/houseInDeleteMulti",
            data:{ids:ids},
            dataType:"json",
            beforeSend:function(){
                debugger;
...
function reQuery(){
    document.location.assign("/productinstore/houseInQueryLikeKeys?"+"keys=&activePage=1"+"&startNum=1&pageSize="+$("#selectValue").val());
}
...
function queryByKeys(){
    ...
    document.location.assign("/productinstore/houseInQueryLikeKeys?"+"keys="+$("#search-input").val()+"&activePage=1"+"&startNum=1&pageSize="+$("#selectValue").val());
