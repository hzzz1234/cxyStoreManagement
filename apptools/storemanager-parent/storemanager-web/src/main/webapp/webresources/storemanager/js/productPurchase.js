 ...
 * 最后修正：2009-12-18
 **/
var IMAGESPATH = '../webresources/storemanager/images/'; //图片路径配置
	...
	' + (function (obj) {
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: center; background-color:#f6f6f6;">\
                <input type="button" class="buttonStyle" value="保 存" id="_ButtonOK_' + this.ID + '"/>&nbsp&nbsp&nbsp&nbsp\
...
function deleteOne(id){
    ...
    if(!confirm("确认要删除？,数据不可恢复！")){
        ...
        $.ajax({
            type:"GET",
            url:"/productpurchase/purchaseDelete",
            data:{id:id},
            dataType:"json",
            beforeSend:function(){
                debugger;
...
function addul(parentid,innerHtml)
{
    var parentFrame=document.getElementById(parentid);
    var ulNode=document.createElement("div");
    ulNode.id=parentid+"-temp-div";
    ulNode.innerHTML=innerHtml;
    parentFrame.appendChild(ulNode);
}
function deleteAllmessage(parentid,childrenid){
    var parent=document.getElementById(parentid);
    var child=document.getElementById(childrenid);
    if(child!=null){
